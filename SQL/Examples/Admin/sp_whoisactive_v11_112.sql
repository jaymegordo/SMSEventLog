...
BEGIN;
	...
	BEGIN;
				...
				(
					SELECT TOP(@i)
						...
						(
							...
							(DATEADD(second, -(tx.ms_ticks / 1000), GETDATE()))
						) AS start_time,
						sp.login_time,
						CASE
					...
					(
						...
						(
							...
							WHERE
								w0.last_wait_type = ''dirty_page_poll''
						) AS w
						OUTER APPLY
						(
							SELECT TOP(1)
								CASE
									WHEN w.start_quantum > w.wait_started_ms_ticks THEN w.start_quantum
									WHEN w.wait_started_ms_ticks > w.wait_resumed_ms_ticks THEN w.wait_started_ms_ticks
									ELSE w.wait_resumed_ms_ticks
								END + 
									ISNULL(t0.wait_duration_ms, 0)
							FROM sys.dm_os_waiting_tasks AS t0
							WHERE
								t0.waiting_task_address = w.task_address
						) AS t (ms_ticks)
					) AS tx
				) AS y
				' + 
									...
									(
										...
										(
											...
											(
												...
												(
													SELECT TOP(@i)
														tx.ms_ticks - w0.wait_resumed_ms_ticks AS runnable_time,
														w0.worker_address,
														w0.thread_address,
		...
		BEGIN;
			SELECT TOP(1)
				@first_collection_ms_ticks = t.ms_ticks
			FROM
			(
				SELECT TOP(1)
					w0.*
