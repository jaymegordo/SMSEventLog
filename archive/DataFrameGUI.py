"""
DataFrameTable
Compatible with PyQt5 & pd==0.25.2
==============
Quick and Dirty Qt app to view pd DataFrames. Includes sorting and
filterting.
Based on qtpandas in pd sandbox module, by Jev Kuznetsov
Usage:
...
class WidgetedCell(object):
    """Set as the value of an element in a pd DataFrame to create a widget
...
class DataFrameModel(QtCore.QAbstractTableModel):
    ...
    def sort(self, col_ix, order = QtCore.Qt.AscendingOrder):
        ...
        self.df = self.df.sort_values(self.df.columns[col_ix], ascending=True)
...
class DynamicFilterMenuAction(QtWidgets.QWidgetAction):
    ...
    def __init__(self, parent, menu, col_ix):
        ...
        Args:
            ...
            col_ix (int)
                Index of column used in pd DataFrame we are to filter
        """
    ...
    def __init__(self, parent, menu, col_ix):
        ...
        Args:
            ...
            menu (QMenu)
                Menu object I am located on
            col_ix (int)
                Column index used in pd DataFrame we are to filter
            label (str)
                Label in popup menu
        """
...
class DataFrameWidget(QtWidgets.QTableView):
    ...
    def __init__(self, parent=None, df=None):
        """DataFrameTable
        Create a widget to display a pd DataFrame.
        Args:
            parent (QObject)
    ...
    def make_cell_context_menu(self, menu, row_ix, col_ix):
        ...
        """
        cell_val = self.df.iloc[row_ix, col_ix]
...
class DataFrameApp(QtWidgets.QMainWindow):
    ...
    def __init__(self, df, title="Inspecting DataFrame"):
        ...
        self.setGeometry(0, 0, col_size + 200, 1000)
...
if __name__ == '__main__':
    ...
    df = pd.DataFrame(df, columns=['AAA','BBB','CCC','DDD','EEE','FFF','GGG','HHH','III'])
    app = DataFrameApp(df)
    app.show()
    _app.exec_()